...
import os
import pandas as pd
from survey_to_activitysim.utils import logcontroller
import datetime
from survey_to_activitysim.core import geolocate
from survey_to_activitysim.utils.survey_data import SurveyData

pd.options.mode.chained_assignment = None  # default='warn'


def locate_parcels(config, survey_data: SurveyData):
    ...
    survey_data.trips.set_index("trip_id", inplace=True)
    ...
    hh_filter_dict_list = [
        {
            ...
            "hh_filter": (-survey_data.households["home_lat"].isnull()),
        }
    ]

    hh_new = geolocate.locate_hh_parcels(
        survey_data.households.copy(), "household_id", parcel_df, hh_filter_dict_list, config
    )
    ...
    person_orig_update = survey_data.persons.merge(
    ...
    trip_results = survey_data.trips.copy()
    ...
    trip_original_updated = survey_data.trips.merge(
        trip[
            [
                "otaz",
                "dtaz",
